...
def add_regressors(
    regr_index_key, regr_list_key
):  # adiciona um grupo de regressores ao session_state específico
    regkeys = utils.sorter(
        [regk for regk in list(st.session_state) if regr_index_key in regk]
    )  # por segurança, deixa as keys de cada regressor do grupo em ordem alfanumérica
    nested_regressors = [
        st.session_state[regk] for regk in regkeys
    ]  # pega os valores correspondentes de cada key no dict session_state e coloca na lista do grupo de regressores
    st.session_state[regr_list_key].append(
        nested_regressors
    )  # faz o append da lista do grupo de regressores na lista 'geral' de regressores


def add_thetas(
    thetas_index_key, thetas_list_key
):  # o código é semelhante ao acima, a diferença é a última linha
    ...
    st.session_state[
        thetas_list_key
    ] = nested_thetas  # aqui é feita uma atribuição ao invés de append
...
if "regr_list" not in st.session_state:
    st.session_state[
        "regr_list"
    ] = list()  # lista 'geral' para armazenar todos os grupos de regressores

if "thetas_list" not in st.session_state:
    st.session_state[
        "thetas_list"
    ] = list()  # lista para armazenar os thetas referentes a cada grupo de regressores

with col2:
    st.file_uploader("Test Input Data", key="tx_data", help="Upload your CSV file")
    if st.session_state["tx_data"] != None:
        x_test = pd.read_csv(
            st.session_state["tx_data"], sep="\t", header=None
        ).to_numpy()
...
with col3:
    ...
    if st.session_state["ty_data"] != None:
        y_test = pd.read_csv(
            st.session_state["ty_data"], sep="\t", header=None
        ).to_numpy()

st.markdown("---")

st.write(
    "Define your model (check the [tutorial](https://sysidentpy.org/examples/simulating_a_predefined_model/))"
)

st.write(
    "Insert the regressors codification (the order of the codes must be the same as the one in the tutorial)"
)
...
with col5:
    with st.form("Reg form"):
        for reg_input_number in range(st.session_state["nonl_deg"]):
            st.number_input(
                "Regressor " + str(reg_input_number + 1),
                key="reg_" + str(reg_input_number + 1),
                min_value=0,
                value=0,
            )
    ...
    with st.form("Theta form"):
        for numb_theta in range(model_code_l.shape[0]):
            st.number_input(
                "Parameter value for regressor group " + str(numb_theta + 1),
                key="theta_" + str(numb_theta + 1),
                value=1.0e0,
                format="%e",
            )
...
if st.session_state["tx_data"] != None and st.session_state["ty_data"] != None:
    with col7:
        ...
        with st.form("Simulate"):
            ...
            st.write()
            sim_model = st.form_submit_button(
                "Simulate the model",
                help="This button will work only if your data is loaded and the regressors/parameters are properly set!",
            )
    ...
    if sim_model == True:
        sim = SimulateNARMAX(
            ...
            calculate_err=st.session_state["calc_err"],
            estimate_parameter=st.session_state["estimate_par"],
            extended_least_squares=st.session_state["extend_least_squares"],
        )
            ...
            for index in range(len(metrics_list)):
                if (
                    ...
                    metrics_vallist.append(
                        getattr(metrics, metrics_list[index])(y_test, yhat_sim)
                    )

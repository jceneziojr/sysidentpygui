...
if platform.system() == "Linux":
    sys.path.append(
        os.path.join(os.path.dirname(__file__).replace("/tests", "") + "/assist")
    )
else:
    sys.path.append(
        os.path.join(os.path.dirname(__file__).replace("\\tests", "") + "\\assist")
    )
    ...
    def generate_model_struc_selec_parameter_list(self):
        try:
            model_struc_selec_parameter_list = (
                utils.get_model_struc_selec_parameter_list(model_structure_selection)
            )
        except:
            return False
        else:
